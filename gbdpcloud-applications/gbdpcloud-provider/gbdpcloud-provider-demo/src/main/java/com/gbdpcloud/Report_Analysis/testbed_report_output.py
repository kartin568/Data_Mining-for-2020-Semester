...
testbed_mts_filename = "C:\\LDRA_Workarea\\"+planid+"_tbwrkfls\\"+planid+".mts.htm"
rts_filename = "C:\\LDRA_Workarea\\"+planid+"_tbwrkfls\\"+planid+".rps.htm"
print(testbed_mts_filename, "\n", rts_filename,"\n", tester,"\n", software,"\n", version,"\n", test_date,"\n", test_tool)
#print(type(testbed_rts_filename), type(rts_filename), type(tester), type(software), type(version), type(test_date), type(test_tool))
#静态质量度量
function_count = []
function_list = []
function_flag_com = 0
...
file = open(testbed_mts_filename, 'r')
soup = bs(file, 'lxml')
...
for d in file_names.keys():
    tmp_file_names.append(d)
...
'''
o = 0
for a in range(len(function_count)):
    for b in range(len(function_list[a])):
        tab.cell(line + 1+ o, 0).text = str(o+1)
        tab.cell(line + 1+ o, 1).text = str(tmp_file_names[a])+":"+tmp_func[o]
        tab.cell(line + 1+ o, 3).text = str(tmp_co[o])
        tab.cell(line + 1+ o, 4).text = str(tmp_fan[o])
        o = o+1
...
front_save = "D:\\N_S_T_P\\StaticTestPlatform-master\\public\\documents\\"+planid+".docx"
doc.save(front_save)